...
var GameEngine = {
    ...
    launch: function(opts) {
        ...
        this.width = 1536/2;//window.innerWidth;
        this.height = 2048/2;//window.innerHeight-GameEngine.bannerHeight;
		this.ow=this.width;
		this.oh=this.height;
    ...
    getReady: function() {
		...
		if(!GameEngine.readyPrompt){

        var ready = document.createElement("DIV");
        ready.innerHTML = "<br>Get Ready!<br><br>";
			var menubtn = document.createElement("DIV");
        menubtn.classList.add("ok");
			menubtn.innerHTML="Menu";
			menubtn.style.marginBottom = "6px";
        var ok = document.createElement("DIV");
        ok.classList.add("ok");
        ok.innerHTML = "Ok";
        ready.appendChild(menubtn);
        ready.appendChild(ok);
		    	var readyprmt = new GameEngine.prompt(ready,"ready");

        ok.addEventListener("click", function() {

            if(GameEngine.readyPrompt)
            GameEngine.readyPrompt.style.display = "none";
				if(GameEngine.readyPrmpt)
            GameEngine.readyPrmpt.style.display = "none";
            GameEngine.paused = false;
        });
			GameEngine.readyPrompt=readyprmt.prompt;
		}else{
            GameEngine.readyPrompt.style.display = "block";
		}
	...
	continueLoss: function(){
		if(!GameEngine.readyPrmpt){
        var ready = document.createElement("DIV");
        var span = document.createElement("SPAN");
        var shop = document.createElement("DIV");
        shop.classList.add("ok");
			shop.innerHTML="Shop";
			shop.style.marginBottom = "6px";
			var menubtn = document.createElement("DIV");
        menubtn.classList.add("ok");
			menubtn.innerHTML="Menu";
			menubtn.style.marginBottom = "6px";
        ...
        ready.appendChild(shop);
        ready.appendChild(menubtn);
        ready.appendChild(ok);
				GameEngine.readyPrmptSP = span;
				GameEngine.readyPrmptOK = ok;
...
GameEngine.lost=false;
        ...
        shop.addEventListener("click", function() {
            ...
            window.location.href="./index.html";
				...
				if(GameEngine.setuplose)
			GameEngine.setuplose(GameEngine.readyPrmptSP,GameEngine.readyPrmptOK);
				            GameEngine.readyPrmpt.style.display = "block";
...
shopSkinsPrompt.prompt.addEventListener("click", function() {
			hiderShow(function(){
            window.location.href="./shopEnviro.html";
			});
        });

var shopSkinsChild = createDiv(false,"PowerUps");
...
var shopSkinsChild = createDiv(false,"NoAds");
...
var shopSkinsChild = createDiv(false,"skinshop");
...
var continueChild = createDiv(false,false,"Play Again");
	...
	continuePrompt.prompt.addEventListener("click", function() {
						GameEngine.lost=false;
		
            GameEngine.continuePrompt.style.display = "none";
			GameEngine.mainPrompt.style.display = "none";
			if(GameEngine.reset)
			GameEngine.reset();
            GameEngine.paused = false;
			
        });
		GameEngine.mainPrompt = mainPrompt.prompt;

	GameEngine.continuePrompt = continuePrompt.prompt;
}else{
	GameEngine.continuePrompt.style.display = "block";
	GameEngine.mainPrompt.style.display = "block";
}
